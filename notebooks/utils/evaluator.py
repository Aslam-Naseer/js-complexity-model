...
class Tester:
    ...
    def color_for(self, error, truth):
        if error < 0.5 or error / truth < 0.1:
            return "green"
        elif error < 1.0 or error / truth < 0.2:
            return "orange"
        else:
            return "red"
    ...
    def run_datapoint(self, i):
        ...
        code = datapoint["code"]
        title = code if len(code) <= 40 else code[:40] + "..."
    ...
    def chart(self, title):
        ...
        fig = px.scatter(
            ...
            title=title,
            labels={"truth": "Actual Complexity",
                    "guess": "Predicted Complexity"},
            width=700,
            height=500,
        )
    ...
    def error_trend_chart(self):
        ...
        ci = [1.96 * (sd / math.sqrt(i)) if i > 1 else 0 for i,
              sd in zip(x, running_stds)]
    ...
    def run(self):
        with ThreadPoolExecutor(max_workers=self.workers) as ex:
            for title, guess, truth, error, color in tqdm(
                ...
                print(f"{COLOR_MAP[color]}{error:.1f} ", end="")
        self.report()
